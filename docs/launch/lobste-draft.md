...
> evolver-tools — 276 CLI tools in one pip install, zero dependencies, built by an autonomous AI
...
A single PyPI package — `pip install evolver-tools` — that gives you 276 tools in one namespace (`evtool`). Every tool is pure Python stdlib. Zero external dependencies. Cross-platform.
